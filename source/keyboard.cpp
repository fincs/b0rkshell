...
static void drawIndicator(color_t* bmpBuf, int x, int y)
{
	for (int j = 0; j < 4; j ++)
		for (int i = 0; i < 4; i ++)
...
void CKeyboard::SetDrawBuffer(color_t* newBuf)
{
	if (!newBuf) return;
	bmpBuf = newBuf;
	renderLabels();
...
ssize_t CKeyboard::SyncRead(char* buf, size_t len)
{
	readBuf = buf;
	readBufPos = 0;
	readBufLen = len;
...
int CKeyboard::getKey(const TouchPos& pos)
{
	const keydef_t* keyMap = getKeyMap(bShifted);
...
int CKeyboard::OnClick(const TouchPos& pos)
{
	int k = getKey(pos);
	if (k < 0)
		return -1;
...
static CGrf* keybdGfx;
static int keybdGfxRefCount;

const GrfFile* CKeyboard::GetGraphics()
{
	return keybdGfx;
}

static void releaseGfx()
{
	if (!--keybdGfxRefCount && keybdGfx)
	{
		delete keybdGfx;
		keybdGfx = nullptr;
	}
...
IKeyboard* newKeyboard(const char* layout)
{
	...
	if (!keybdGfxRefCount++)
	{
		...
		if (!keybdGfx->Load(GUI_ASSET_DIR "/keyboard.grf"))
		{
			releaseGfx();
			return nullptr;
		}
